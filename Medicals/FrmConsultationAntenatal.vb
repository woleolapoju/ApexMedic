...
Public Class FrmConsultationAntenatal
    ...
    Private bindsourceAC As New BindingSource()
    Private PatientNo As String

    Private Sub FrmConsultationMini_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        DbGridVitals.DataSource = bindsource
        DbGridVitals.AutoGenerateColumns = False

        GetData("select VitalSign,'' AS Result from VitalSignType order by VitalSign")

        DbGridAC.DataSource = bindsourceAC
        DbGridAC.AutoGenerateColumns = False

        GetDataAC("SELECT Items AS Item,'' AS Result FROM AntenatalConsultationElements ORDER BY Sn")
        ...
        If DistributeToConsultingRoom = True Then
            lblPostTo.Text = "Consulting Room"
            FillConsultingRoom()
        Else
...
handler:
    ...
    Private Sub FillConsultingRoom()
        ...
        Do While drSQL.Read
            ...
            If DistributeToConsultingRoom = True Then cDoctor.Items.Add(drSQL.Item("Descr").ToString)
        Loop
...
FetchNoAgain:
        If Action <> AppAction.Delete Then
            ...
            End If
        End If
        If Trim(tCDetails.Text) = "" And listXray.Items.Count < 1 And listScan.Items.Count < 1 And listLab.Items.Count < 1 Then
        ...
        For i = 0 To DbGridVitals.Rows.Count - 1
            If Trim(DbGridVitals.Item(0, i).Value) <> "" And Trim(DbGridVitals.Item(1, i).Value) <> "" Then SystemicExamSummary = SystemicExamSummary + IIf(SystemicExamSummary <> "", Chr(13), "") + DbGridVitals.Item(0, i).Value + " ( " + DbGridVitals.Item(1, i).Value + " )"
        Next i

        For i = 0 To DbGridAC.Rows.Count - 1
            If Trim(DbGridAC.Item(0, i).Value) <> "" And Trim(DbGridAC.Item(1, i).Value) <> "" Then SystemicReviewSummary = SystemicReviewSummary + IIf(SystemicReviewSummary <> "", Chr(13), "") + DbGridAC.Item(0, i).Value + " ( " + DbGridAC.Item(1, i).Value + " )"
        Next i
        ...
        Select Case Action
            Case AppAction.Add
                ...
                cmSQL.Parameters.AddWithValue("@Diagnosis", " ")
                If UseRequestForm Then
                ...
                cmSQL.Parameters.AddWithValue("@Note", tCDetails.Text)
                ...
                cmSQL.Parameters.AddWithValue("@MedicationSummary", tPrescription.Text)
                cmSQL.Parameters.AddWithValue("@PresentingComplaintCSummary", " ")
                cmSQL.Parameters.AddWithValue("@HistoryOfPresentingComplaint", " ")
                cmSQL.Parameters.AddWithValue("@TreatmentHistory", " ")
                cmSQL.Parameters.AddWithValue("@SystemicReviewSummary", SystemicReviewSummary)
                cmSQL.Parameters.AddWithValue("@GeneralExamination", " ")
                cmSQL.Parameters.AddWithValue("@SystemicExamSummary", SystemicExamSummary)
                cmSQL.Parameters.AddWithValue("@Procedures", " ")
                ...
                For i = 0 To DbGridAC.Rows.Count - 1
                    If Trim(DbGridAC.Item(0, i).Value) <> "" And Trim(DbGridAC.Item(1, i).Value) <> "" Then
                        cmSQL.Parameters.Clear()
                        cmSQL.CommandText = "InsertConsultationSystemicReview"
                        cmSQL.CommandType = CommandType.StoredProcedure
                        cmSQL.Parameters.AddWithValue("@RefNo", tRefNo.Text)
                        cmSQL.Parameters.AddWithValue("@SystemicReview", DbGridAC.Item(0, i).Value)
                        cmSQL.Parameters.AddWithValue("@Result", DbGridAC.Item(1, i).Value)
                        cmSQL.ExecuteNonQuery()
                    End If
                Next i

                For i = 0 To DbGridVitals.Rows.Count - 1
                    If Trim(DbGridVitals.Item(0, i).Value) <> "" And Trim(DbGridVitals.Item(1, i).Value) <> "" Then
                        cmSQL.Parameters.Clear()
                        cmSQL.CommandText = "InsertConsultationSystemicExam"
                        cmSQL.CommandType = CommandType.StoredProcedure
                        cmSQL.Parameters.AddWithValue("@RefNo", tRefNo.Text)
                        cmSQL.Parameters.AddWithValue("@SystemicExam", DbGridVitals.Item(0, i).Value)
                        cmSQL.Parameters.AddWithValue("@Result", DbGridVitals.Item(1, i).Value)
                        cmSQL.ExecuteNonQuery()
                    End If
                Next i
...
errhdl:
    ...
    Private Function oLoad(ByVal strCode As String) As Boolean
        ...
        Do While drSQL.Read = True
            ...
            tCDetails.Text = ChkNull(drSQL.Item("Note"))
...
TryAgainXray:
        ...
        Dim strQuery As String = "SELECT SystemicReview AS Item, Result,0 AS Sn FROM ConsultationSystemicReview WHERE RefNo='" & strCode & "' UNION SELECT Items AS Item,'' AS Result, Sn FROM AntenatalConsultationElements WHERE  NOT (Items IN (SELECT SystemicReview FROM ConsultationSystemicReview WHERE RefNo='" & strCode & "') ) ORDER BY Sn"
        GetDataAC(strQuery)

        strQuery = "SELECT SystemicExam AS VitalSign, Result FROM ConsultationSystemicExam WHERE RefNo='" & strCode & "' UNION SELECT VitalSign, '' AS Result FROM VitalSignType WHERE  NOT (VitalSign IN (SELECT SystemicExam FROM ConsultationSystemicExam WHERE RefNo='" & strCode & "'))"
        GetData(strQuery)
...
handler:
    ...
    Private Sub PrintResult(ByVal LastRefNo As String)
        ...
        report = New ConsultationAntenatal
        ...
        Dim SelFormular As String = " {RptConsultation.RefNo}='" + LastRefNo & "'"
...
errhdl:
    ...
    Private Sub cmdNewICD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
...
errhdl:
    ...
    Private Sub mnuConsultation_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuConsultation.Click
        If ReturnPatient = "" Then
            ...
            Exit Sub
        End If
        Dim ChildForm As New FrmConsultationHistory
        ChildForm.PatientNo = ReturnPatient
        ChildForm.ShowDialog()
    End Sub
    ...
    Private Sub lnkVitalSign_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles lnkVitalSign.LinkClicked
        PanVital.Left = lnkPostPatient.Left 'Me.Width / 2
        PanVital.Top = Me.Height / 2
        PanVital.Visible = Not PanVital.Visible
        PanPostPatient.Visible = False
    End Sub

    Private Sub cmdCloseVitals_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdCloseVitals.Click
...
handler:
    ...
    End Sub
    Private Sub cmdClosePostPatient_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdClosePostPatient.Click
        PanPostPatient.Visible = False
    End Sub
    Private Sub lnkPostPatient_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles lnkPostPatient.LinkClicked
        If Trim(tPatientNo.Text) = "" Or Trim(tPatientName.Text) = "" Then
            MsgBox("Pls. select patient to post...", MsgBoxStyle.Information, strApptitle)
            Exit Sub
        End If
        PanPostPatient.Left = lnkPostPatient.Left 'Me.Width / 2
        PanPostPatient.Top = Me.Height / 2
        PanPostPatient.Visible = Not PanPostPatient.Visible
        PanVital.Visible = False
    End Sub
...
errhdl:
    ...
    Private Sub GetDataAC(ByVal selectCommand As String)
    ...
    Private Sub lnkBooking_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles lnkBooking.LinkClicked
