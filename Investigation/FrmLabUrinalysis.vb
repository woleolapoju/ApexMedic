...
Public Class FrmLabUrinalysis
    Public ReturnStaffName, ReturnStaffNo As String
    Public frmParent As Object

    Private Sub FrmLabUrinalysis_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
FetchNoAgain:
        If lblAction.Text <> "Delete Record" Then
            ...
            If Val(Trim(tTestNo.Text)) <= 0 Then
                MsgBox("TestNo should be a numeric and greater than Zero (0)", MsgBoxStyle.Information, strApptitle)
                Exit Sub
            End If
        End If
        ...
        Select Case lblAction.Text
            Case Is = "New Record"
                ...
                cmSQL.CommandText = "InsertLabResultUrinalysis"
                cmSQL.CommandType = CommandType.StoredProcedure
                ...
                cmSQL.Parameters.AddWithValue("@SG", cSG.Text)
                cmSQL.Parameters.AddWithValue("@PH", cPH.Text)
                cmSQL.Parameters.AddWithValue("@Glucose", cGlucose.text)
                cmSQL.Parameters.AddWithValue("@Proteins", cProteins.Text)
                cmSQL.Parameters.AddWithValue("@Ketones", cKetones.Text)
                cmSQL.Parameters.AddWithValue("@Nitrites", cNitrites.Text)
                cmSQL.Parameters.AddWithValue("@Bilirubin", cBilirubin.Text)
                cmSQL.Parameters.AddWithValue("@Urobilinogen", cUrobilinogen.Text)
                cmSQL.Parameters.AddWithValue("@EpithCells", cEpithCells.Text)
                cmSQL.Parameters.AddWithValue("@YeastCells", cYeastCells.Text)
                cmSQL.Parameters.AddWithValue("@WBC", tWBC.Text)
                cmSQL.Parameters.AddWithValue("@RBC", tRBC.Text)
                cmSQL.Parameters.AddWithValue("@Crystals", tCrystals.Text)
                cmSQL.Parameters.AddWithValue("@Casts", tCasts.Text)
                cmSQL.Parameters.AddWithValue("@Haematobium", cHaematobium.Text)
                ...
                For i = 0 To DbGrid.Rows.Count - 1
                    If DbGrid.Item(0, i).Value <> Nothing And Trim(DbGrid.Item(0, i).Value) <> "" Then
                        If Not ((DbGrid.Item(1, i).Value = "N/A" Or DbGrid.Item(1, i).Value = Nothing) And (DbGrid.Item(2, i).Value = "N/A" Or DbGrid.Item(2, i).Value = Nothing) And (DbGrid.Item(3, i).Value = "N/A" Or DbGrid.Item(3, i).Value = Nothing)) Then
                            cmSQL.Parameters.Clear()
                            cmSQL.CommandText = "InsertLabResultUrinalysisCultureDetails"
                            cmSQL.Parameters.AddWithValue("@TestNo", Val(tTestNo.Text))
            ...
            Case Is = "Edit Record"
                ...
                cmSQL.CommandText = "DeleteLabResultUrinalysis"
                cmSQL.CommandType = CommandType.StoredProcedure
                cmSQL.Parameters.AddWithValue("@TestNo", Val(tTestNo.Text))
                cmSQL.ExecuteNonQuery()
...
handler:
    ...
    Private Function FetchNextNo() As Boolean
        ...
        cmSQL.CommandText = "SELECT ISNULL(MAX(TestNo), 0) +1 AS NewNo FROM LabResultUrinalysis"
...
errhdl:
    ...
    Private Sub oLoad(ByVal TestNo As Integer)
        ...
        cmSQL.CommandText = "SELECT * FROM LabResultUrinalysis WHERE TestNo=" & TestNo
        cmSQL.CommandType = CommandType.Text
        cnSQL.Open()
        drSQL = cmSQL.ExecuteReader()
        Do While drSQL.Read
            ...
            cSG.Text = drSQL.Item("SG")
            cPH.Text = drSQL.Item("PH")
            cGlucose.Text = drSQL.Item("Glucose")
            cProteins.Text = drSQL.Item("Proteins")
            cKetones.Text = drSQL.Item("Ketones")
            cNitrites.Text = drSQL.Item("Nitrites")
            cBilirubin.Text = drSQL.Item("Bilirubin")
            cUrobilinogen.Text = drSQL.Item("Urobilinogen")
            cEpithCells.Text = drSQL.Item("EpithCells")
            cYeastCells.Text = drSQL.Item("YeastCells")
            tWBC.Text = drSQL.Item("WBC")
            tRBC.Text = drSQL.Item("RBC")
            tCrystals.Text = drSQL.Item("Crystals")
            tCasts.Text = drSQL.Item("Casts")
            cHaematobium.Text = drSQL.Item("Haematobium")

        Loop

        drSQL.Close()
        Dim TheQry As String = "SELECT Drug, Organism1, Organism2, Organism3 FROM LabResultUrinalysisCultureDetails WHERE TestNo=" & TestNo & _
                            " UNION SELECT Drug,'' AS Organism1,'' AS Organism2,'' AS Organism3 FROM LabCultureDrugs WHERE Drug NOT IN (SELECT Drug FROM LabResultUrinalysisCultureDetails WHERE TestNo=" & TestNo & ")"
