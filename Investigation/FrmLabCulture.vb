...
Public Class FrmLabCulture
    Public ReturnStaffName, ReturnStaffNo As String
    Public frmParent As Object
    Private Sub FrmLabCulture_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub oLoad(ByVal TestNo As Integer)
        ...
        cmSQL.CommandText = "SELECT * FROM LabResultCulture WHERE TestNo=" & TestNo
        cmSQL.CommandType = CommandType.Text
        cnSQL.Open()
        drSQL = cmSQL.ExecuteReader()
        Do While drSQL.Read
            ' cmSQL.Parameters.AddWithValue("@TestNo", Val(tTestNo.Text))
            'cmSQL.Parameters.AddWithValue("@PatientNo", tPatientNo.Text=drSQL.Item("PatientNo")
        ...
        Dim i As Integer
        drSQL.Close()
        cmSQL.CommandText = "SELECT * FROM LabResultCultureGram WHERE TestNo=" & TestNo
        cmSQL.CommandType = CommandType.Text
        drSQL = cmSQL.ExecuteReader()
        Do While drSQL.Read
            ' Find the string in lstGram.
            Dim index As Integer = lstGram.FindString(drSQL.Item("Item"))
            ' If the item was not found in ListBox 2 display a message box, otherwise select it in ListBox2.
            If index = -1 Then
                lstGram.Items.Add(drSQL.Item("Item"))
                lstGram.SetSelected(lstGram.Items.Count - 1, True)
            Else
                lstGram.SetSelected(index, True)
            End If
        Loop

        drSQL.Close()
        cmSQL.CommandText = "SELECT * FROM LabResultCultureWet WHERE TestNo=" & TestNo
        cmSQL.CommandType = CommandType.Text
        drSQL = cmSQL.ExecuteReader()
        Do While drSQL.Read
            ' Find the string in lstWet.
            Dim index As Integer = lstWet.FindString(drSQL.Item("Item"))
            ' If the item was not found in ListBox 2 display a message box, otherwise select it in ListBox2.
            If index = -1 Then
                lstWet.Items.Add(drSQL.Item("Item"))
                lstWet.SetSelected(lstWet.Items.Count - 1, True)
            Else
                lstWet.SetSelected(index, True)
            End If
        Loop

        drSQL.Close()
        Dim TheQry As String = "SELECT Drug, Organism1, Organism2, Organism3 FROM LabResultCultureSensitivity WHERE TestNo=" & TestNo & _
                            " UNION SELECT Drug,'' AS Organism1,'' AS Organism2,'' AS Organism3 FROM LabCultureDrugs WHERE Drug NOT IN (SELECT Drug FROM LabResultCultureSensitivity WHERE TestNo=" & TestNo & ")"
...
handler:
    ...
    Private Sub FillGram()
        ...
        lstGram.Items.Clear()
        lstGram.Items.Add("-Not Applicable-")
        cmSQL.CommandText = "FetchCultureGram"
...
handler:
    ...
    Private Sub FillWet()
        ...
        lstWet.Items.Clear()
        lstWet.Items.Add("-Not Applicable-")
        cmSQL.CommandText = "FetchCultureWet"
        cmSQL.CommandType = CommandType.StoredProcedure
        cnSQL.Open()
        drSQL = cmSQL.ExecuteReader()
        Do While drSQL.Read
...
FetchNoAgain:
        ...
        Select Case lblAction.Text
            Case Is = "New Record"
                ...
                cmSQL.CommandText = "InsertLabResultCulture"
                cmSQL.CommandType = CommandType.StoredProcedure
                cmSQL.Parameters.AddWithValue("@TestNo", Val(tTestNo.Text))
                ...
                For i = 0 To lstGram.Items.Count - 1
                    If lstGram.GetSelected(i) = True Then
                        cmSQL.Parameters.Clear()
                        cmSQL.CommandText = "InsertLabResultCultureGram"
                        cmSQL.CommandType = CommandType.StoredProcedure
                        cmSQL.Parameters.AddWithValue("@TestNo", Val(tTestNo.Text))
                        cmSQL.Parameters.AddWithValue("@Item", lstGram.Items.Item(i))
                        cmSQL.ExecuteNonQuery()
                    End If
                Next

                For i = 0 To lstWet.Items.Count - 1
                    If lstWet.GetSelected(i) = True Then
                        cmSQL.Parameters.Clear()
                        cmSQL.CommandText = "InsertLabResultCultureWet"
                        cmSQL.CommandType = CommandType.StoredProcedure
                        cmSQL.Parameters.AddWithValue("@TestNo", Val(tTestNo.Text))
                        cmSQL.Parameters.AddWithValue("@Item", lstWet.Items.Item(i))
                        cmSQL.ExecuteNonQuery()
                    End If
                Next
                ...
                For i = 0 To DbGrid.Rows.Count - 1
                    If DbGrid.Item(0, i).Value <> Nothing And Trim(DbGrid.Item(0, i).Value) <> "" Then
                        If Not ((DbGrid.Item(1, i).Value = "N/A" Or DbGrid.Item(1, i).Value = Nothing) And (DbGrid.Item(2, i).Value = "N/A" Or DbGrid.Item(2, i).Value = Nothing) And (DbGrid.Item(3, i).Value = "N/A" Or DbGrid.Item(3, i).Value = Nothing)) Then
                            cmSQL.Parameters.Clear()
                            cmSQL.CommandText = "InsertLabResultCultureSensitivity"
                            cmSQL.Parameters.AddWithValue("@TestNo", Val(tTestNo.Text))
            ...
            Case Is = "Edit Record"
                        ...
                        cmSQL.CommandText = "DeleteLabResultCulture"
                        cmSQL.CommandType = CommandType.StoredProcedure
                        cmSQL.Parameters.AddWithValue("@TestNo", Val(tTestNo.Text))
                        cmSQL.ExecuteNonQuery()
...
handler:
    ...
    Private Function FetchNextNo() As Boolean
        ...
        cmSQL.CommandText = "SELECT ISNULL(MAX(TestNo), 0) +1 AS NewNo FROM LabResultCulture"
        cmSQL.CommandType = CommandType.Text
        cnSQL.Open()
        drSQL = cmSQL.ExecuteReader
